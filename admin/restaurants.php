 ...
 * Single file handling list, add, edit, and delete operations for restaurants
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add' || $action === 'edit') {
            ...
            if (empty($name) || !in_array($type, ['restaurant', 'cafe'])) {
                ...
                if ($action === 'add') {
                    ...
                    if ($stmt->execute()) {
                        ...
                        if ($create_owner) {
                            ...
                            if ($check_stmt->get_result()->num_rows > 0) {
                                $error = 'Owner email already exists. Restaurant was added but owner account was not created.';
                            } else {
                                ...
                                if ($owner_stmt->execute()) {
                                    $message = 'Restaurant and owner account added successfully!';
                                } else {
                                    $message = 'Restaurant added successfully, but failed to create owner account.';
                                }
                                $owner_stmt->close();
                            }
                            $check_stmt->close();
                    ...
                    if ($check_result->num_rows > 0) {
                        $old_restaurant = $check_result->fetch_assoc();
                        $old_logo = $old_restaurant['logo'] ?? '';
                    }
                    $check_stmt->close();
                    ...
                    $stmt = $conn->prepare("UPDATE restaurants SET name = ?, type = ?, location_in_airport = ?, description = ?, is_visible = ?, logo = ? WHERE id = ?");
                    $stmt->bind_param("ssssisi", $name, $type, $location, $description, $is_visible, $final_logo, $id);
            ...
            $logo_file = '';
            if ($check_result->num_rows > 0) {
                $restaurant = $check_result->fetch_assoc();
                $logo_file = $restaurant['logo'] ?? '';
            }
            $check_stmt->close();
...
$restaurants = [];
$query = "SELECT * FROM restaurants WHERE type = 'restaurant' ORDER BY name";
...
if ($action === 'edit' && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $stmt = $conn->prepare("SELECT * FROM restaurants WHERE id = ? AND type = 'restaurant'");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $edit_restaurant = $result->fetch_assoc();
    } else {
        $action = 'list';
...
<?php if ($action === 'list'): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (!empty($restaurant['logo'])): ?>
                                <img src="/uploads/<?php echo htmlspecialchars($restaurant['logo']); ?>" 
                                     alt="Logo" 
                        ...
                        <td><?php echo htmlspecialchars($restaurant['location_in_airport'] ?? 'N/A'); ?></td>
                        <td><?php echo htmlspecialchars(substr($restaurant['description'] ?? '', 0, 50)) . '...'; ?></td>
                        <td>
                            <?php if (isset($restaurant['is_visible']) && $restaurant['is_visible'] == 1): ?>
                        ...
                        <td>
                            <a href="?action=edit&id=<?php echo $restaurant['id']; ?>" class="btn btn-sm btn-primary">Edit</a>
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <?php if ($action === 'edit'): ?>
            <input type="hidden" name="id" value="<?php echo $edit_restaurant['id']; ?>">
        <?php endif; ?>
        
        <div class="mb-3">
            <label for="name" class="form-label">Restaurant Name *</label>
            <input type="text" class="form-control" id="name" name="name" 
                   value="<?php echo htmlspecialchars($edit_restaurant['name'] ?? ''); ?>" required>
        </div>
        
        <div class="mb-3">
            <label for="type" class="form-label">Type *</label>
            <select class="form-control" id="type" name="type" required>
                <option value="restaurant" <?php echo ($edit_restaurant['type'] ?? 'restaurant') === 'restaurant' ? 'selected' : ''; ?>>Restaurant</option>
                <option value="cafe" <?php echo ($edit_restaurant['type'] ?? '') === 'cafe' ? 'selected' : ''; ?>>Cafe</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="location_in_airport" name="location_in_airport" 
                   value="<?php echo htmlspecialchars($edit_restaurant['location_in_airport'] ?? ''); ?>">
        </div>
        
        <div class="mb-3">
            <label for="description" class="form-label">Description</label>
            <textarea class="form-control" id="description" name="description" rows="3"><?php echo htmlspecialchars($edit_restaurant['description'] ?? ''); ?></textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="hidden" name="logo" value="<?php echo htmlspecialchars($edit_restaurant['logo'] ?? ''); ?>">
            <?php if (isset($edit_restaurant['logo']) && !empty($edit_restaurant['logo'])): ?>
                <div class="mt-2">
                    <small class="text-muted">Current logo:</small><br>
                    <img src="/uploads/<?php echo htmlspecialchars($edit_restaurant['logo']); ?>" 
                         alt="Restaurant logo" 
        ...
        <div class="mb-3">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" id="is_visible" name="is_visible" value="1" 
                       <?php echo (!isset($edit_restaurant['is_visible']) || $edit_restaurant['is_visible'] == 1) ? 'checked' : ''; ?>>
                <label class="form-check-label" for="is_visible">
                    Visible to public
                </label>
                <small class="form-text text-muted d-block">Uncheck to hide this restaurant from public pages</small>
            </div>
        </div>
        ...
        <button type="submit" class="btn btn-primary"><?php echo $action === 'add' ? 'Add Restaurant' : 'Update Restaurant'; ?></button>
